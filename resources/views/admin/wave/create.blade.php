...
@section('title', "Buat Gelombang Baru")
...
<div class="p-10">
    ...
    <form class="bg-white rounded-lg shadow-lg p-10 mt-8" method="POST" action="{{ route('admin.wave.store') }}">
        @csrf
        <div class="text-xs text-slate-500">Nama Gelombang</div>
        <input type="text" name="name" class="w-full outline-0 h-14 border text-sm px-6 mt-2" placeholder="Gelombang I - Jalur Umum {{ date('Y') }}" required>

        <div class="text-xs text-slate-500 mt-8">Biaya</div>
        <input type="text" id="price_display" name="price_display" class="w-full outline-0 h-14 border text-sm px-6 mt-2" oninput="changePrice(this)" required>
        ...
        <div class="flex items-center gap-4 mt-8">
            <div class="flex flex-col gap-2 grow">
                ...
                <div class="flex items-center gap-4 px-4 border">
                    ...
                    <input type="text" name="start_date" class="w-full outline-0 h-14 text-sm" id="start_date" required readonly>
                </div>
            </div>
            <div class="flex flex-col gap-2 w-6/12 hidden" id="endDateArea">
                <div class="text-xs text-slate-500">Tanggal Berakhir</div>
                <div class="flex items-center gap-4 px-4 border">
                    <ion-icon name="calendar-outline"></ion-icon>
                    <input type="text" name="end_date" class="w-full outline-0 h-14 text-sm" id="end_date" required readonly>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-4">
            <input type="number" min="1" id="qty" name="quantity" class="w-full outline-0 h-14 border text-sm px-6 mt-2" value="10" required>
        ...
        <div class="flex items-center justify-end gap-4 mt-12">
            ...
            <button class="text-xs p-3 px-6 font-medium bg-primary text-white">Tambahkan</button>
        </div>
    </form>
</div>
@endsection
...
<script>
    ...
    select("#price_display").value = Currency(0).encode();
